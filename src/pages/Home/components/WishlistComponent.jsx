...
export const WishlistComponent = ({ user }) => {
    ...
    useEffect(() => {
        const fetchWishlists = async () => {
            try {
                const response = await axios.get(`https://server2-acel.onrender.com/api/wishlists/${user._id}`);
    ...
    const addToCart = async (item) => {
        try {
            const response = await axios.post(`https://server2-acel.onrender.com/api/carts/addToCart`, {
    ...
    const removeFromWishlist = async (item, wishlistId) => {
        try {
            ...
            const response = await axios.delete(`https://server2-acel.onrender.com/api/wishlists/${wishlistId}/removeProduct/${item._id}`);
            if (response.status === 200) {
