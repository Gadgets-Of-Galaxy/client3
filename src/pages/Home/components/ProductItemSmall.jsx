...
export const ProductItemSmall = (props) => {
    ...
    const handleSaveWishlist = async () => {
        if (createNewWishlist) {
            try {
                const response = await axios.post(`https://server2-acel.onrender.com/api/wishlists/create/${user._id}`, {
                    name: wishlistName,
                });
    ...
    const addProductToWishlist = async (wishlistId) => {
        try {
            const response = await axios.post(`https://server2-acel.onrender.com/api/wishlists/addProduct/${wishlistId}`, {
                productId: productId,
                imagePath: imagePath,
                price: price,
